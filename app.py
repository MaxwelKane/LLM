...
# # --- Header ---
# st.html("""<h2 style="text-align: center;">📚🔍 <i> DISC Helper </i> 🤖💬</h2>""")
...
if missing_openai and missing_anthropic and ("AZ_OPENAI_API_KEY" not in os.environ):
    ...
    with st.sidebar:
        ...
        with cols0[1]:
            ...
            st.toggle(
                "Use Pinecone", 
                value=st.session_state.use_pinecone,
                key="use_pinecone",
                disabled=not pinecone_configured,
                help="Requires PINECONE_API_KEY in environment" if not pinecone_configured else "Use Pinecone for context retrieval"
            )
        ...
        # Show current vector store being used
        if st.session_state.get("use_rag", False):
            if st.session_state.get("use_pinecone", False):
                st.info("🔍 Using Pinecone for context retrieval")
            else:
                st.info("🔍 Using ChromaDB for context retrieval")
        ...
        if st.session_state.get("use_pinecone", False):
            ...
            st.info(f"📌 Pinecone Index: {pinecone_index}")
